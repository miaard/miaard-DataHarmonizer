  ...
  */
  async openJSONDataFile(file) {
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      /**
       * The JSON array of row objects sparse, so only keys with values are
       * included. Search through a table's row objects to accumulate a full
       * list of their keys. The col_index provides a ROUGH idea of which 
       * column to claim for a given key, though a sparse array may vary from
       * row to row depending on required/optional fields.
       */
      const header = [];
      ...
      const matches = header.filter((element, index) => dh.slot_names.includes(element) || dh.slot_titles.includes(element)).length;
      // raw_data only contains templates that match incomming data tables.  
  ...
  loadTabularData(file, raw_data) {
    ...
    Object.entries(this.context.dhs).forEach(([template_name, dh]) => {
      ...
      if (data.matches != data.header.length && data.matches != dh.slots.length) { 
