...
class Toolbar {
  ...
  createNewFile(e) {
    ...
    if (e.target.id === 'new-dropdown-item' && isNotEmpty) {
      ...
      for (let [dependent_name] of this.context.relations[dh.template_name].dependents.entries()) {
        this.clearDH(this.context.dhs[dependent_name]);
      }
      this.clearDH(dh);
  ...
  */
  async openJSONDataFile(file) {
    ...
    if (matches) {
      alert(`The JSON data file's metadata is not compatible for loading! Missing fields: ${matches}`);
      return false;
    };
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      // JSON data is sparse, so only keys with values are included.
      // Search through a table to accumulate a full list of its fields.
      // The col_index provides a rough idea of which column to claim for a
      // given key, though a sparse array may vary from row to row.
      const header = [];
      Object.entries(data).forEach(([row_index, row]) => {
        Object.entries(row).forEach(([key, col_index]) => {
          if (!(header.includes(key))) {
            header.splice(col_index, 0, key);
          }
        });
      });

      const matches = dh.slot_names.filter((element, index) => header.includes(element)).length;
      // raw_data only contains templates which were matched to incomming data
      // tables.  
    ...
    // Future: make a generic modal for this:
    if (messages.length) {
      alert(messages.join('\n'))
    }
  ...
  async openTabularDataFile(file) {
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      console.log("TAB HEADER", matrix[header_row-1])
  ...
  loadTabularData(file, raw_data) {
    ...
    Object.entries(this.context.dhs).forEach(([template_name, dh]) => {
      ...
      if (dh.slots.length !== data.matches) { 
        // Have user try to correct any missing mappings in a class/template. 
  ...
  /**
   * Value is looked up as enumeration text or title if possible.
   * value_list is merged schema.enums[x].permissible_values structure.
   */
