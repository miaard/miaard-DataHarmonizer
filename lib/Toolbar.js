...
import { MULTIVALUED_DELIMITER, dataObjectToArray} from '../lib/utils/fields';
import { takeKeys, invert } from '../lib/utils/objects';
import { findBestLocaleMatch, LocaleNotSupportedError } from '../lib/utils/templates';
import { utils as XlsxUtils, read as xlsxRead} from 'xlsx/xlsx.js';
import { FieldMapper, compareMatrixHeadersToGrid } from '../lib/FieldMapper';
...
class Toolbar {
   ...
   */
  clearDH(class_name, preserve_settings = false) {
  ...
  */
  async openJSONDataFile(file) {
    ...
    // The JSON file should have one or more tables in it that match to loaded schema.
    // The data is possibly *sparse*.
    Object.entries(this.context.dhs).forEach(([dh_name, dh]) => {
      ...
      const cleaned_data = dh.matrixFieldChangeRules(data_table);
  ...
  check_locale(language = 'en') {
    if (language !== i18next.language) {
      i18next.changeLanguage(language === 'en' ? 'default' : language);
      const translationSelect = $('#select-translation-localization');
      translationSelect.val(i18next.language === 'en' ? 'default' : language);
      $(document).localize();
    }
  }
  ...
  async openTabularDataFile(file) {
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      // Calculate REAL headerRow - usually it is 2nd row.
      const [header_row, slot_count, matches] = compareMatrixHeadersToGrid(dh, matrix);
     ...
     if (header_row < 1 || matches === 0) {
        messages.push(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO header row OR NO matching fields with the corresponding data file tab or table. This DH tab table will be empty.\n`);
        continue;
      }
      
      // raw_data only contains templates which were matched to incomming
      // data tables.  Data starts at calculated header row.
      raw_data[dh.template.name] = {
  ...
  loadTabularData(file, raw_data) {
    ...
    // FieldMapper uses file.name just for display, and raw_data for
    // hot.loadData() of revisions
    ...
    for (const dh of Object.values(this.context.dhs)) {
      let data = raw_data[dh.template.name];

      this.clearDH(dh.template.name);  // Does away with dh settings.
      if (data.slot_count == data.matches) { 
        // Here all DH tab schema slots have matched so it is a clean load fieldwise.
        console.log("Loading spreadsheet tab", data.table_name, "into", dh.template.name);
        // Returns data by chopping 1 or 2 header rows off

        const cleaned_data = dh.matrixFieldChangeRules(data.data);
        dh.hot.loadData(cleaned_data);
        continue;
      } 

      // Have user try to correct any missing mappings. 
      fieldMapper.appendFieldMappingModal(dh);

    }

    if (fieldMapper.field_mapping_html) {
      console.log("HTML", fieldMapper.field_mapping_html)
      ...
      // How to get fieldMapper to return the mapping and do loadData() here?
      //  .then(() => {alert(mapping); console.log(mapping)})
  ...
  async saveDataFile() {
    ...
    const container_datasets = this.getContainerData(ext);
    ...
    for (const [attribute_name, attribute] of Object.entries(container_attributes)) {
      ...
      for (const row of dh.hot.getSourceData()) {
        ...
        row.forEach((value, index) => {
          ...
          if (!isEmptyUnitVal(value)) { // 'undefined' / null / '';
            ...
            if (slot.sources) {
              ...
              if (slot.multivalued === true) {
                ...
                .map((_value) => {
                  // AppContext.js slot.merged_permissible_values is dictionary
                  // of enum permissible value entries.
                  return this.getValueByNameOrTitle(_value, ext, slot.merged_permissible_values); 
                });
                ...
                new_value = this.getValueByNameOrTitle(value, ext, slot.merged_permissible_values);
              }
            }
          ...
          // For tabular data, this could be made an option in the future.
          row_dict[ext == 'json' ? slot_name : slot.title] = new_value;
  ...
  // Value list is merged schema.enums[x].permissible_values structure.
  getValueByNameOrTitle (value, file_extension, value_list) {
    ...
    if (value_list[value].title)
      return value_list[value].title;

    return value_list[value].text;
