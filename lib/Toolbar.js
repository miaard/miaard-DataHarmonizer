...
import {isEmpty, deleteEmptyKeyVals, isEmptyUnitVal} from '../lib/utils/general';
...
import FieldMapper from '../lib/FieldMapper';
...
//            alert(`Unable to fetch table data from JSON file {template_path} for {dh}`);
  ...
  async openTabularDataFile(file) {
    ...
    // FieldMapper uses file.name just for display, and workbook for
    // hot.loadData() of revisions
    const fieldMapper = new FieldMapper(this.context, file.name, workbook);
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      if (!sheet_name) {
        alert(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO corresponding table in the uploaded data file.  The data file's tabs or tables are: "${workbook.SheetNames}".  Consequently, no data will be loaded.  Either a new table has been added to schema, or the wrong schema is loaded.`);
      }
      else {

        const worksheet = updateSheetRange(workbook.Sheets[sheet_name]);
        // Load data with expectation of header (but we recalculate where
        // header is via data_fields below):
        ...
        const [header_row, schema_slot_count, matches] = fieldMapper.compareMatrixHeadersToGrid(dh, matrix);
       ...
       if (header_row < 1 || matches === 0) {
          alert(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO header row OR NO matching fields with the corresponding data file tab or table. This DH tab table will be empty.`);
          continue;
        }
        
        // In all cases the real data starts at calculated header row.
        let data_table = matrix.slice(header_row);
        const data_fields = matrix[header_row-1];

        if (schema_slot_count == matches) { 
        ...
        // Have user try to correct any missing mappings. They are not 
        // forced to specify all field mappings, some can be left empty
        // and so no data will be loaded for them.
        ...
        fieldMapper.appendFieldMappingModal(dh, data_fields, sheet_name);
          ...
          if (!isEmptyUnitVal(value)) { // 'undefined' / null / '';
            ...
            if (slot.sources) {
              ...
              if (slot.multivalued === true) {
                new_value = value.split(MULTIVALUED_DELIMITER)
                .map((_v) => {
                  // AppContext.js slot.merged_permissible_values is dictionary
                  // of enum permissible value entries.
                  return this.getValueByNameOrTitle(_v, ext, slot.merged_permissible_values); 
                });
            ...
            */
            new_value = dh.setJSON(new_value, slot.datatype);
