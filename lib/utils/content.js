...
export const urlToClickableAnchor = (string) => {
  ...
  uniqueMatches.forEach((url) => {
    ...
    const regex = new RegExp(escapedURL.replace(RE_URLCHARS, '\\$&'), 'g');
...
function formatEscapeHTML(string) {
  return string.replace(RE_ESCAPE, function(char) {return ESCAPE[char]});
};
