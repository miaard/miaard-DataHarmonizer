...
import { deleteEmptyKeyVals } from './general';
...
export function exportWorkbook(workbook, baseName, ext) {
  ...
  // Spreadsheets get saved in one file with different tabs.
  if (ext == 'xlsx' || ext == 'xls') {
      writeFile(workbook, `${baseName}.${ext}`);
      return;
  }

  // For TSV, CSV, Often just one sheet, but if multiple, then each gets file
  // name + _ + template (class) name
    ...
    switch (ext) {
      ...
      */
      ...
      case 'csv': // UTF-8
        ...
        var data = XlsxUtils.sheet_to_csv(worksheet, {FS: ','});
        data = '\uFEFF' + data; //BOM
      ...
      *
      case 'tsv (UTF-16)': // no BOM
        ...
        var data = XlsxUtils.sheet_to_txt(worksheet, {FS: '\t'});
        saveBlob(data, fileName, 'text/plain;charset=UTF-16LE');
        break;
      */
    }
...
/* OBSOLETING: DH experimental data format was tabular.
For conversion load tabular json dataset in 1.6.5 and resave.
...
export function importJsonFile(jsonData) {
  return processJsonBasedOnType(jsonData,
    (data) => {
...
// ONLY USED IN Toolbar.js saveTabularDataFile().
// Main function to handle the entire process of prepending row to sheets
// Useful for custom binning headers like sections
export function modifySheetRow (workbook, sheetName, valueCellPairs, targetRow) {

  const worksheet = workbook.Sheets[sheetName];
  const data = XlsxUtils.sheet_to_json(worksheet, { header: 1 });
  // -1 signals to insert a new line
  if (targetRow !== undefined && targetRow === -1) {
    data.unshift([]); // Adds new first row
    targetRow = 0;
  }
  if (targetRow === undefined || targetRow >= data.length) {
    data.push([]); // Adds new last row
  ...
  workbook.Sheets[sheetName] = XlsxUtils.aoa_to_sheet(data);
...
export function prependRowWithValuesAcrossSheets(workbook, valueCellPairs) {
  ...
  newWorkbook.SheetNames.forEach((sheetName) => {
    modifySheetRow(newWorkbook, sheetName, valueCellPairs[sheetName], -1);
  });
  return newWorkbook;
}
*/
