...
const SCHEMAMENUS = ['SchemaSlotTypeMenu','SchemaClassMenu','SchemaSlotMenu','SchemaSlotGroupMenu','SchemaEnumMenu'];
...
export default class AppContext {
  ...
  getSchemaRef() {return this.template?.default?.schema}
  ...
  async tabChange(class_name) {
    ...
    if (this.getCurrentDataHarmonizer() !== class_name) {
      ...
      // Changing tabs clears out validation for that tab.
  ...
  refreshTabDisplay() {
    ...
    $('#slot_report_type, #slot_report_type option')
      .toggle(dh.schema.name === 'DH_LinkML' && class_name === 'Slot');
    ...
    switch (report_type) {
      case 'all':
        this.tabFilter(dh, {}); // gets rid of any filters
...
*/
  ...
  tabFilter(dh, key_vals) {
    ...
    for (let row = 0; row < dh.hot.countSourceRows(); row++) {
      let found = true;
      Object.entries(key_vals).forEach(([key_name, value]) => {
        let column = dh.slot_name_to_column[key_name];
        if (dh.hot.getDataAtCell(row, column) !== value)
          found = false;
      })
      if (found)
        shown.push(row);
    ...
    hiddenRowsPlugin.showRows(shown);
    hiddenRowsPlugin.hideRows(hidden); 
...
*/
  ...
  slotTypeFilter(dh, show_type = null) {
    ...
    for (let row = 0; row < dh.hot.countSourceRows(); row++){
      if (show_type) {
        let slot_type = dh.hot.getSourceDataAtCell(row, this.slot_type_column);
        if (show_type.includes(slot_type)) {
          shown.push(row)
        }
        else {
          hidden.push(row)
      ...
      else { // default is to show all fields
        shown.push(row)
      }
    ...
    hiddenRowsPlugin.showRows(shown);
    hiddenRowsPlugin.hideRows(hidden); 
    //console.log("Showhide",shown,hidden)
    //dh.hot.resumeExecution();
    dh.render(); // Otherwise chunks of old html left with old visible rows
  }
  ...
  async reload(template_path, locale = null, forced_schema = null) {
    ...
    // Disable export if no export.js content
    $('#export-to-dropdown-item').toggleClass("disabled", isEmpty(this.export_formats));
      ...
      for (const enum_name of enums) {
        ...
        switch (enum_name) {
          ...
          case 'SchemaSlotGroupMenu':
            ...
            if (class_name) {
              filter.class_id = class_name;
            }

            const slot_rows = this.crudFindAllRowsByKeyVals('Slot', filter);
  ...
  getSchemaEditorSelectedSchema() {
    const dh = this.dhs['Schema'];
    return dh.hot.getDataAtCell(dh.current_selection[0], 0);
  }

  getSchemaEditorSelectedClass() {
    const dh = this.dhs['Class'];
   ...
   * This has HTML construction going on inside it.  This should be separated 
   * out into a rendering function.
   * 
      ...
      .forEach((obj, index) => {
        if (obj.length > 0) {
          ...
          let tooltip = Object.keys(this.relations[class_name]?.parent)
            .map((parent_name) => schema.classes[parent_name].title || schema.classes[parent_name].name)
            .join(', ');
          if (tooltip.length) {
          ...
          dhTab.addEventListener('click', (event) => { 
            ...
            // Disabled tabs do not triger dhTabChange.
            if (event.srcElement.classList.contains("disabled"))
              return false;

            // When using handsontable filter(), a timing thing causes 
            // HandsonTable not to render header row(s) and left sidebar 
            // column(s) on tab change unless we do a timed delay on 
            // tabChange() call.
            setTimeout(() => { this.tabChange(class_name)}, 200);
            //this.tabChange(class_name);
            return false;
          });
          ...
          // If this class_name is a range of a Container attribute,
          // then create an easy reference to container name
          if (class_name != 'Container' && 'Container' in schema.classes) {
            let found_name = Object.values(schema.classes.Container.attributes)
              .filter((v) => v.range && v.range === class_name)?.[0]?.name;
            if (found_name)
          ...
          if (is_child) {
            ...
            if (schema.name === 'DH_LinkML' && class_name === 'Slot') {

              handsOnDH.slot_type_column = handsOnDH.slot_name_to_column['slot_type'];
              handsOnDH.slot_class_name_column = handsOnDH.slot_name_to_column['class_name'];
              handsOnDH.slot_name_column = handsOnDH.slot_name_to_column['name'];
              handsOnDH.slot_rank_column = handsOnDH.slot_name_to_column['rank'];
              handsOnDH.slot_group_column = handsOnDH.slot_name_to_column['slot_group'];
            }
  ...
  /** If a given Handsontable [column_name] cell value equals [value] 
   * (e.g. slot_type column == 'slot') then set its metadata className, which
   * is visible to css stylesheets, 
   * For ReadOnly too.
   * "The setCellMeta() method will add the meta once and keep it once cells are moved to other position and won’t change it on render" - https://forum.handsontable.com/t/gh-6274-best-place-to-set-cell-meta-data/4710
   * https://github.com/handsontable/handsontable/issues/6274

  setRowCSSonColValue(dh, column_name, value_map) {
    let col = dh.slot_name_to_column[column_name];      
    for (let row = 0; row < dh.hot.countRows(); row++){
      let value = dh.hot.getDataAtCell(row, col);
      let className = value_map[value];
      if (className)
  ...
  useTemplate(dh, template_name) {
    ...
    // A class has slots list used just to include Schema slots, and a slot_usage dict which provides extended attributes to the schema slots. But both these are "compiled" by either tabular_to_json.py or linkml.py SchemaView() call into the class's attributes dictionary.
    const raw_attributes = this.template.default.schema.classes[dh.template_name].attributes;
    //console.log("Attributes(", template_name,")", raw_attributes);

    // Attributes need to be organized by slot group, and then rank within those slot groups, before processing below.

    // NMDC created slots having same name as slot_groups, and giving each a rank, to control ordering.

    /* Need to determine order of slot_usage items, in order to sort attributes in same order.
     * 1st guess of order comes by first encounter in attributes.  Slot_usage doesn't necessarily
     * mention order of every slot.  However it does indicate tailoring content such as ordering
     * to given class.
     * Mixins get wovent into attribute slots, e.g.
     *  "mixins": [
    ...
    let sections = {};
    let count = 0;
    for (let name in raw_attributes) {
      let slot_group = raw_attributes[name].slot_group;
      if (!slot_group) {
        slot_group = 'Field'; //Make a default slot_group to gather any unnamed ones.
        raw_attributes[name].slot_group = slot_group;
      }
      if (!sections[slot_group]) {
        // Templates can declare a slot_group as a slot in order to control ranking
        // May want to qualify this as NMDC did with slot "is_a":"dh_section"
        // ISSUE: a spec can mention a slot_group as a Title of a slot!
        if (this.template.default.schema.slots[slot_group]?.rank) {
          
          sections[slot_group] = this.template.default.schema.slots[slot_group].rank;
        }
        else {
          // OTHERWISE First encountered slot_group gets priority
          count ++;
          sections[slot_group] = count;
        }
      }
    }
    //console.log("Sections", sections)

    const attributes = Object.fromEntries(Object.entries(raw_attributes).sort((a, b) => sections[a[1].slot_group] - sections[b[1].slot_group] || a[1].rank - b[1].rank));

    //console.log("Attributes(", template_name,")", attributes);

    for (name in attributes) {
      //console.log("SLOT", name, attributes[name], this.template.current.schema.slots[name])
      /** ISSUE: tabular_to_schema.py is not compiling locale fields into class attributes. It 
       * only implements SchemaView() inheritance. So deepMerge goes and gets Schema level
       * slot definition which has locale stuff in it.
       * FUTURE: simplify to just language overlay.
       */
      let slot = deepMerge(
        attributes[name],
  ...
  setDHSlotLookups(dh) {
    ...
    // Sort kids in each section by rank given in LinkML class's slot_usage.
    ...
    dh.slots = dh.sections.map((section) => section.children).flat(1);

    // LOOKUP: column # to name
    dh.slot_names = dh.slots.map((slot) => slot.name);
    dh.slot_titles = dh.slots.map((slot) => slot.title);
   ...
   * "range" slot's .sources and permissible_values object and related
  ...
  setSlotRangeLookup(slot) {
    ...
    for (let range of slot.sources) {
      ...
      slot.permissible_values??= {}; // Assigns new object only if undefined.
      slot.permissible_values[range] = permissible_values;

      // For convenience, precalculates flat list of permissible values for use
      // in getContainerData. 
      slot.merged_permissible_values??= {};
      Object.assign(slot.merged_permissible_values, permissible_values);
  ...
  * Recursively flatten vocabulary into an array of strings, with each
  ...
  stringifyNestedVocabulary(vocab_list) {
    ...
    for (const pointer in vocab_list) {
      ...
      /* PHASING THIS OUT - depth/level now handled in DataHarmonizer.js 
       * updateSources(slot).
      let level = 0;
      if ('is_a' in choice) {
        level = stack.indexOf(choice.is_a) + 1;
        stack.splice(level + 1, 1000, choice.text); // 1000 -> all subsequent text.
      } else {
        stack = [choice.text];
      }
      */
      ret.push(choice.text);
    }
    return ret;
  }
  ...
  */
  ...
  /**
   * In Display section (top of page), show cookiecrumb path from root to current
  ...
  crudUpdateRecordPath() {
    ...
    while (stack.length >0) {
      ...
      if (class_name in done)
        continue;
      const class_label = schema.classes[class_name].title || schema.classes[class_name].name;
      ...
      let tooltip_text = `<span class="tooltipytext">${tooltip} </span>`;
      done[class_name] = `<span class="tooltipy"><b>${class_label}:</b>&nbsp;${key_string}${tooltip_text}</span> `;
  ...
  crudFilterDependentRows(class_name, dependent_report) {
    ...
    // this class's foreign keys if any, are partially or fully satisfied.
   ...
   *  0: full foreign key not satisfied
   *  1: foreign key partly satified...
   *  2: no foreign keys (ergo, satisfied)
   *  3: foreign keys satisfied,
   *  
   * OLD:
   *  1: no foreign keys (ergo, satisfied)
   *  2: foreign keys satisfied, 
   * 
   * // Not implemented: remaining unique key slots unsatisfied
  ...
  crudGetDependentKeyVals(class_name, changes) { 
    ...
    let fkey_vals = {};
    let key_vals = {};
    let fkey_status = 3; // Assume fks satisfied
    ...
    if (class_dh) { // FUTURE: avoid this case altogether.
      ...
      // If no foreign keys, then key satisfied
      if (this.relations[class_name].foreign_key_count === 0) {
        fkey_status = 2;
      }
      else {
        // Here one or more foreign (dependent) key slots exist.
        ...
        for (let [slot_name, slot_link] of Object.entries(dependent_slots)) {
          ...
          if (!dependent_f_rows) {
            console.log(`No dependent foreign row links for ${class_name}:${slot_name} slot on dependent slots: `, dependent_slots);
            break;
          }
          ...
          if (dependent_f_rows.fkey_status == 0) {
            fkey_status = 0;
            break; // Exit loop, final fkey_status set.
          }
        ...
        // Signal partially complete fkey.
        if (fkey_status === 0 && found_fkey)
          fkey_status = 1;
      }
