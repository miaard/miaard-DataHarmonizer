...
const SCHEMAMENUS = ['SchemaDataTypeMenu','SchemaClassMenu','SchemaSlotMenu','SchemaSlotGroupMenu','SchemaEnumMenu'];
    ...
    if (schema.name === 'DH_LinkML') {
      SCHEMAMENUS.forEach((item) => {
        if (!(item in schema.enums)) {
          // permissibleValues are computed when a particular schema is loaded.
    ...
    Object.entries(schema.enums).forEach(([enum_name, enum_obj]) => {
      if (enum_obj.inherits) {
        ...
        enum_obj.inherits.forEach(inherit_enum_name => {
          Object.assign(values, schema.enums[inherit_enum_name].permissible_values); 
        });
        // Order is important here - given enum_obj must be last.
        enum_obj.permissible_values = {...values, ...enum_obj.permissible_values};
      }
    });
