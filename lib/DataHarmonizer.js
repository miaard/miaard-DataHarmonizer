...
const SCHEMAMENUS = ['SchemaClassMenu','SchemaSlotMenu','SchemaSlotTypeMenu','SchemaSlotGroupMenu','SchemaEnumMenu'];
...
//        }
    ...
    $('#field-mapping-modal').on('hidden.bs.modal', () => {
      ...
      for (let dep_row of rows) {
        ...
        for (let [dep_col, dep_slot] of Object.entries(dependent_dh.slots)) {
          ...
          if (value !== undefined &&  value !== '') { //.length > 0 // took out !!value - was skipping numbers.
            ...
            record[dep_slot.name] = this.setJSON(value, dep_slot.datatype);
          }
        }
...
<body>
  ...
  getSpreadsheetName(workbook) {
    ...
    if (sheet_names.includes(this.template.name))
      ...
      sheet_name = this.template.title; // Language dependent
    }
