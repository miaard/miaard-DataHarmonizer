...
export class FieldMapper {
  constructor(context, file_name, workbook) {

    // References AppContext as created via Toolbar.js
    this.context = context;
    this.workbook = workbook; // Used to map columns on specific DH's on save. 
    this.workbook_tabs = {} // A lookup of class_name to workbook tab.
  ...
  saveMappedData() {
    ...
    // Loop through mapping's list of tables for which there is a discordance.
    Object.entries(mappings?.tables || {}).forEach(([table_name, map_obj]) => {

      const dh = this.context.dhs[table_name];
      //.workbook_tabs[dh.template_name] = sheet_name;
      const worksheet = updateSheetRange(this.workbook.Sheets[this.workbook_tabs[table_name]]);
      const matrix = XlsxUtils.sheet_to_json(worksheet, {header: 1, raw: false, range: 0});
      // Calculate REAL headerRow - usually it is 2nd row.
      const [header_row] = this.compareMatrixHeadersToGrid(dh, matrix);
      const data_table = matrix.slice(header_row); 
   ...
   */
  appendFieldMappingModal(dh, data_fields, sheet_name) {

    // Lookup function used when actually shifting headers.
    this.workbook_tabs[dh.template_name] = sheet_name;
   ...
   * profile name be associated with this.
   */
  getProfileMapping() {
    ...
    $('table#field-mapping-table tbody[data-table]').each((t_index, tbody) => {
      ...
      $(tbody).find('td:first-child.field-mismatch').each((index, slot_field) => {
        ...
        if (data_label.length > 0) {// target has a mapping in it.
          let row_map = {
            'from': get_label($(slot_field).text()),
            'to': data_label
          };
