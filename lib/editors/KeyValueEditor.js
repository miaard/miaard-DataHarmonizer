...
export default class KeyValueListEditor extends Handsontable.editors
  ...
  prepare(row, col, prop, td, value, cellProperties) {
    ...
    function filter(e, controller) {
      ...
      controller.menu = controller.TEXTAREA.nextElementSibling;
      ...
      if (!controller.oldTD || controller.oldTD !== controller.TD) {
        ...
        controller.choices = controller.menu.querySelectorAll('td');
      }
      ...
      const table = controller.menu.querySelector('table.htCore');

      // This height is a constant reflecting total height of menu window. 
      const menu_height = $(controller.menu).height();
      const table_height = $(table).height();
      //const junk = table.offsetHeight; // Forces browser renderer to complete css above.
      //console.log("TABLE HEIGHT", table_height, junk)

      const input_top = $(controller.TEXTAREA).offset().top;
      const input_height = $(controller.TEXTAREA).height();
      ...
      //console.log(dh_height, '+', dh_top, '(', dh_height + dh_top , ')', input_top, input_height, table_height, '(', input_top + input_height + table_height, ')');
      if ((dh_height + dh_top) < (input_top + menu_height)) {
        $(table).css('position','fixed').css('top', (input_top - input_height - table_height) + 'px');
      }
      else {
