...
export default class KeyValueListEditor extends Handsontable.editors
  ...
  prepare(row, col, prop, td, value, cellProperties) {
    ...
    // Prepare is run as soon as user clicks on menu. Determine if menu should
    // be positioned above or below given row.
    ...
    function filter(e, controller) {
      ...
      if (controller.menu_above) {
        $(controller.filtered_table).css('top', (controller.input_top - $(controller.filtered_table).height() ) + 'px');
      }
  ...
  focus() {
    ...
    if (this.menu_above) {
      ...
      //const junk = this.filtered_table.offsetHeight; //triggers re-render
      $(this.filtered_table).css('top', (this.input_top - $(this.filtered_table).height()) + 'px');
    }
  }
